...
#'     \item \code{"pr_below_alpha"} (default): Among candidate thresholds for
...
#'     \item \code{"ftr"}: First-time-rejection rule. Select the first
#'       threshold at which the GOF null hypothesis is accepted
#'       (\code{p\_value > gof_alpha}).
#'     \item \code{"ftr_min5"}: Variant of \code{"ftr"} requiring that the GOF
#'       null is accepted for at least 5 consecutive thresholds.
#'     \item \code{"fwd_stop"}: ForwardStop-type rule based on transformed GOF
#'       p-values.
#'     \item \code{"gof_cp"}: Changepoint detection on GOF p-values (with added
#'       pseudo p-values) to identify a shift from non-GPD to GPD behavior.
